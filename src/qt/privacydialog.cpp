                                                          ...
                                                          currentBalance(-1)
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_20_ZEROCOIN_MAINTENANCE_MODE)) {
        ...
        ui->pushButtonMintzPIV->setToolTip(tr("zOPCX is currently disabled due to maintenance."));

        ui->pushButtonSpendzPIV->setEnabled(false);
        ui->pushButtonSpendzPIV->setToolTip(tr("zOPCX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_pushButtonSpendzPIV_clicked()
{
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, send zOPCX
        sendzPIV();
        return;
    }
